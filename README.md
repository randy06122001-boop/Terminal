...
- **Multi-Strategy Backtesting:** Fast, vectorized backtesting using `vectorbt` with support for multiple strategy engines (EMA Crossover, RSI Mean Reversion, MACD Momentum, and Bollinger Bands).
...
graph TD
    ...
    API --> VectorBT[Backtesting Engine: vectorbt]
...
- **`BT <TICKER> <STRATEGY>`**: e.g., `BT MSFT RSI` - Runs a vectorized backtest for the specified ticker. If no strategy is provided, it defaults to EMA. Supported: `EMA`, `RSI`, `MACD`, `BB`.
...
- **Quantitative Libraries:** `PyPortfolioOpt`, `vectorbt`, `backtrader`, `vaderSentiment`
